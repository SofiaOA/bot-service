...
public class CallMeConversation extends Conversation {
    ...
    public CallMeConversation(StatusBuilder statusBuilder,
            ApplicationEventPublisher eventPublisher) {
        this.statusBuilder = statusBuilder;
        this.eventPublisher = eventPublisher;
        createMessage(CALLME_CHAT_START, new MessageHeader(Conversation.HEDVIG_USER_ID, -1, false),
                new MessageBodySingleSelect("Hej {NAME}, ska jag ringa dig på {PHONE_NUMBER}?",
                        Lists.newArrayList(new SelectOption("Ja", CALLME_PHONE_OK),
                                new SelectOption("Nej", CALLME_PHONE_CHANGE))));

        createMessage(CALLME_PHONE_OK, new MessageHeader(Conversation.HEDVIG_USER_ID, -1, true),
                new MessageBodySingleSelect("Ok då ser jag till att någon ringer dig?", Lists
                        .newArrayList(SelectLink.toDashboard("Ok", "callme.phone.dashboard"))));

        createMessage(CALLME_PHONE_CHANGE, new MessageHeader(Conversation.HEDVIG_USER_ID, -1, true),
    ...
    private void endConversation(UserContext userContext, Message m) {
        eventPublisher.publishEvent(new RequestPhoneCallEvent(userContext.getMemberId(),
                m.body.text, userContext.getOnBoardingData().getFirstName(),
